...
int main(int argc, char* argv[]){
    ...
    if(verbose){
        ...
        printf("Encoded stream size (with header): %d bytes\n",bytesW + num_unique*(4+1)+6);
        printf("Compression rate: %.02f %% \n",(float)(1-(float)(bytesW + num_unique*(4+1)+6)/num_total)*100);

        float average_len=0.0;
        // for(int i=0;i<num_leafnintern;i++){
        //     if(nd_Huff[nd_idx[i]].type==LEAF){
        //         average_len+=  (float)nd_Huff[nd_idx[i]].num_occur*8/num_total;
        //     }
        // }
    ...
    if((bytesW + num_unique*(4+1)+5)>num_total){
